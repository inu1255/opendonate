...
function extendsConn(coMysql, logger) {
    ...
    coMysql.execSQL = function(sqls) {
		...
		if (autoTrans)
			pms = pms.then(() => db.runAsync("BEGIN TRANSACTION"));
		for(let sql of sqls){
			pms = pms.then(()=>db.SingleSQL(sql, args));
		}
		pms = pms.then(function(rows) {
			if(autoTrans) 
				return db.runAsync("COMMIT TRANSACTION").then(()=>rows);
			return rows;
		}, function(err) {
			if (autoTrans)
				return db.runAsync("ROLLBACK TRANSACTION").then(() => Promise.reject(err)).catch(() => Promise.reject(err));
