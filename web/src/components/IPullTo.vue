<template>
	<div class="vue-pull-to-wrapper" :style="{ height: wrapperHeight, transform: `translate3d(0, ${diff}px, 0)` }">
		<div v-if="topLoadMethod" v-show="state" :style="{ height: `${topBlockHeight}px`, marginTop: `${-topBlockHeight}px` }" class="action-block">
		...
		<div v-if="bottomLoadMethod" v-show="state" :style="{ height: `${bottomBlockHeight}px`, marginBottom: `${-bottomBlockHeight}px` }" class="action-block">
	...
	methods: {
		...
		actionLoading() {
			...
			if (this.direction === 'down') {
				...
				let pms = this.topLoadMethod.call(this, this.actionLoaded);
				if (pms && typeof pms.then === "function")
					pms.then(this.actionLoaded.bind(this), this.actionLoaded.bind(this, 'err'))
				this.scrollTo(this._topConfig.stayDistance);
			} else {
				this.bottomText = this._bottomConfig.loadingText;
				let pms = this.bottomLoadMethod.call(this, this.actionLoaded);
				if (pms && typeof pms.then === "function")
					pms.then(this.actionLoaded.bind(this), this.actionLoaded.bind(this, 'err'))
